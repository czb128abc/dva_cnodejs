export function getId(prefix = 'uui') {
    const timeStr = `_${(new Date().getTime()) * 1000}`;
    const mathStr = `_${Math.floor(Math.random() * 1001)}`;
    return prefix + timeStr + mathStr;
}
...
export const bindEventHandler = (selector, type, fn, parentSelector) => {
    if (parentSelector) {
        document.querySelector(parentSelector).addEventListener(type, (event) => {
...
export const clickNode = (event) => {
    const sNode = closest(event.target, el => el.classList && el.classList.contains('node'));
    const sNodeInput = document.getElementById('selected-node');
...
export const removeInputs = () => {
    const inputs = Array.from(document.getElementById('new-nodelist').children);
    if (inputs.length > 1) {
        inputs.pop().remove();
    }
};
...
export const addNodes = (orgchart) => {
    ...
    const selectedNode = document.getElementById(document.getElementById('selected-node').dataset.node);

    if (!nodeVals.length) {
        alert('Please input value for new node');
        return;
    }
    const nodeType = document.querySelector('input[name="node-type"]:checked');
